...
class Record(models.Model):
    ...
    live_image = models.ImageField(upload_to="record/live_image/", blank=True, null=True, verbose_name='参戦記録一覧画面に表示する画像')
...
class Design_Setting(models.Model):
    preset_image_id = models.ForeignKey('accounts.Preset_Image',on_delete=models.CASCADE,verbose_name='アプリ内画像',upload_to="account/picture/preset")
    user_image_id = models.ForeignKey('accounts.User_Image',on_delete=models.CASCADE,verbose_name='ユーザー本体の画像',upload_to="account/picture/user") 
    record_id = models.ForeignKey('Record', on_delete=models.CASCADE,verbose_name='参戦記録ID')
